...
signal slide_used(cooldown: float)
...
var is_moving_lane: bool = false
var lane_tween: Tween

# Slide parameters
const SLIDE_DISTANCE: float = 4.0
...
func _ready() -> void:
	...
	position = Vector3(LANES[current_lane + 2], 0.0, 0.0)
...
func _physics_process(delta: float) -> void:
	...
	# Constant forward motion along +Z
	velocity.z = current_speed
	velocity.y = 0.0
	# X is handled via tween when lane shifting, so velocity.x is 0
	velocity.x = 0.0
	
	move_and_slide()
	
	# Update distance in GameManager
...
func _handle_stamina(delta: float) -> void:
	if is_exhausted:
		...
		if exhausted_timer <= 0.0:
			is_exhausted = false
			current_stamina = 1.0 # start recovering from small base
			_emit_stamina()
	else:
		# Recover stamina if not tapping
...
func _handle_speed_decay(delta: float) -> void:
	...
	if time_since_last_tap >= SPEED_DECAY_DELAY:
...
func try_slide() -> void:
	...
	# Sudden dash 4m forward via tween
	var target_z := position.z + SLIDE_DISTANCE
	var slide_tween := create_tween().set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	slide_tween.tween_property(self, "position:z", target_z, SLIDE_DURATION)
	...
	slide_used.emit(SLIDE_COOLDOWN)
...
func change_lane(direction: int) -> void:
	...
	var target_x: float = LANES[current_lane + 2]
...
func _update_visual_hop(delta: float) -> void:
	...
	# Hop frequency proportional to current speed
